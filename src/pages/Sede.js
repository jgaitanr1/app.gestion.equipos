...
import { SedeEntity } from '../Entity/SedeEntity';
...
export const Sede = () => {
    ...
    let empty = SedeEntity;

    const baseUrl = environment.baseUrl + "sede/";
    ...
    const saveProduct = () => {
        setSubmitted(true);
        if (product.nombre && product.direccion) {
            let _products = [...data];
    ...
    const leftToolbarTemplate = () => {
        return (
            <React.Fragment>
                <div className="my-2">
                    <Button label="Registrar Sede" icon="pi pi-plus-circle" className="p-button-outlined p-button-success mr-2" onClick={openNew} />
                </div>
            </React.Fragment>
        )
    }
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <div className="actions">
                <Button title="Modificar Sede" icon="pi pi-pencil" className="p-button-rounded p-button-outlined p-button-success mr-2" onClick={() => editProduct(rowData)} />
            </div>
        );
    }
    ...
    const header = (
        <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
            <h5 className="m-0">Configuración de Sedes</h5>
            ...
            <div className="col-12">
                <div className="card">
                    ...
                    <DataTable ref={dt} value={data} selection={selectedProducts} onSelectionChange={(e) => setSelectedProducts(e.value)}
                        ...
                        <Column field="direccion" header="Direccion" body={direccionBodyTemplate}></Column>
                        <Column body={actionBodyTemplate}></Column>
                    </DataTable>
                    <Dialog visible={EntidadNewDialog} style={{ width: '700px' }} header="Datos de Sede" modal className="p-fluid" footer={productDialogFooterNew} onHide={hideDialogNew}>
                        <div className="field col-12">
                            <div className="formgrid grid">
                                <div className="field col-12 md:col-12">
                                    ...
                                    {submitted && !product.nombre && <small className="p-invalid">El nombre de la sede es requerido.</small>}
                                </div>
                            </div>
                            <div className="formgrid grid">
                                <div className="field col-12 md:col-12">
                                    <label htmlFor="direccion">Direccion</label>
                                    <InputText id="direccion" value={product.direccion} onChange={(e) => onInputChange(e, 'direccion')} required className={classNames({ 'p-invalid': submitted && !product.direccion })} />
                                    {submitted && !product.direccion && <small className="p-invalid">La dirección de la sede es requerida.</small>}
                                </div>
                            </div>
                        </div>
                    </Dialog>
